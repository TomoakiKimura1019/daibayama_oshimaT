...
Private Function KGappend(ByVal fdir As String, ByVal tFilename As String, ByVal id As Integer) As Integer
    'Debug.Print tFilename
    'Debug.Print FTPpathname(tFilename) & "/" & tFilename
End Function
...
'-----------------------------------------------------------------------
Private Function GetTargetFiles(ByRef Files() As String, _
...
'    rc = ftp.Connect(ServerIP, "mbkeisya", "mb2803")  '本物
'End If
...
Set ftp = CreateObject("basp21.FTP")
ftp.OpenLog App.Path & "\log.txt"
'rc = ftp.Connect("60.43.239.36", "chikah", "zbeba+nn")
rc = ftp.Connect("172.16.60.99", "anonymous", "")
If rc = 0 Then
   rc = ftp.Command("CWD /array1/share2")   'ディレクトリ移動
'   rc = ftp.Command("MKD 47")   'ディレクトリ作成

   v = ftp.GetDir("/array1/share2/47/2008", 2) ' ディレクトリ一覧(詳細)
   If IsArray(v) Then
       For Each v2 In v
          Debug.Print v2
       Next
   Else
        rc = ftp.Command("MKD /array1/share2/47/2008/01")   'ディレクトリ作成
   End If
   v = ftp.GetReply()
'   v = ftp.GetDir("/A0001") ' ディレクトリ一覧(ファイル名のみ)
'   If IsArray(v) Then
'       For Each v2 In v
'          Debug.Print v2
'       Next
