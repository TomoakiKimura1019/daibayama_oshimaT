...
Public Const ecMaxPort As Long = 255&               ' 最大ポート番号
...
Public Const ecReadIntervalTimeout = 0&             'Version1.80より初期状態では無効に変更
...
Public Const ecWriteTotalTimeoutMultiplier = 0&     'Version1.80より初期状態では無効に変更

Public Const ecInBufferSize As Long = 2048&
    '入力バッファに制限があるときはその値を標準設定値とするが，無制限の時はecInBufferSizeを標準にする．
Public Const ecXonLim As Long = 128&                'バッファしきい値
Public Const ecXoffLim As Long = 128&               'バッファ最小空き容量
...
'
'Version1.80注記
'BuildCommDCBは，DCBのbaud,parity,data,stopの部分だけではなく，
'ハンドシェーキングまで書き換えちゃうんだね．
'びっくりだ．
Public Declare Function _
...
'RTS,DTRの強制制御
'   Version 1.84以降使用を中止．
'   EscapeCommFunctionはRTS,DTRを強制設定するが，DCBには影響がない．
'   つまりSettingプロパティなどでDCBを読み出し，もう一度書き込むとRTS,DTRの設定が
'   無効になってしまう．
'   情けないことだ．
'   RTS,DTR出力の強制設定は，DCBのfBitFieldsの書き換えで行うこと．
'   EscapeCommFunctionは，単に一時的な強制設定の機能しか持たないことに注意！
Public Declare Function _
