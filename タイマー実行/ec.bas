...
'（３）ｅｃは，アプリケーションの一部として利用する場合に限って，木下隆の許可無く自由
...
'       InBufferCLearをSubからFunctionプロシージャに変更
'   Version 1.80    Dec 25,2003
'       デリミタ文字の種類を増加
'       Settingプロパティを実行するとハンドシェーキングが"N"になるバグを修正
'       ポート番号の最大値を255に拡張
'       送信待機中のフリーズ状態をなくした
'
'   Version 1.81    Jan 11,2004
'       Windows98で動作させると，WriteFileがFalseとなり送信エラーが発生するバグ修正
...
'       ①ボーレートを上げると送信時のタイムアウトの計算値がゼロになり，タイムアウトが無効になるので最小値を50mSにした．
'       ②オープンログファイルに関するプロパティとメソッドの追加．
'           XnotOpenLogプロパティ
'               = False (デフォルト)    旧来通りオープンログファイルを使用する
'               = True                  オープンログファイルを使用しない
'           XdeleteOpenLogFileメソッド
'               Windowsのテンポラリファイルにオープンログファイルがあれば削除する
'       ③Soundプロパティの追加
'
'   Version 1.84    JUN 15,2004
'       Settingプロパティを実行するとRTS,DTR出力の強制設定状態が変化するバグ修正
'           EscapeCommFunctionなんか使うでない！→ecDef参照
'
...
'-----------------------------------
'   XnotOpenLogプロパティ(Version1.83から追加)
'-----------------------------------
'   XnotOpenLogプロパティ
'       = False (デフォルト)    旧来通りオープンログファイルを使用する
'       = True                  オープンログファイルを使用しない
...
'１の時はトラップ可能なエラーを発生．
'ただし，１に設定してエラーが発生したとき，トラップルーティンがないとプログラムを終して変数を
...
' 以下Version1.80で追加
    ETX     As String   ' 03h   ETX テキスト終了
    EOT     As String   ' 04h   EOT 伝送終了
    DLE     As String   ' 10h   DLE データリンクでの拡張
    ETB     As String   ' 17h   ETB 伝送ブロック終了
' -------- ここまで
...
Public Function XdeleteOpenLogFile()
    ...
    If rv <> INVALID_HANDLE_VALUE Then  ' エラー時は無視
...
'-----------------------------------
' Version1.83で追加
' WAVファイルのパスを書き込むと，そのファイルが存在するときは再生します．
' 存在しないときは，何もしないで戻ります．
' 再生は非同期で行われるため，処理はファイルの再生終了を待たずに，直ちに戻ります．
...
'書き込み
'Version1.80 ハンドシェークも変わってしまうバグ修正
Public Property Let Setting(Mode As String)
    ...
    End If
' Version1.80で追加
    ecDef.fDCBtemp = ecDef.fDCB     ' DCBの保存
'----- ここまで
...
'----- ここまで
    ...
    'DCBの書き込み
' Version1.80で追加
    'BuildCommDCBのパラメータに対応するメンバだけを変更する
    ecDef.fDCB.BaudRate = ecDef.fDCBtemp.BaudRate   ' 通信速度
    ecDef.fDCB.Parity = ecDef.fDCBtemp.Parity       ' パリティ
    ecDef.fDCB.ByteSize = ecDef.fDCBtemp.ByteSize   ' データビット長
    ecDef.fDCB.StopBits = ecDef.fDCBtemp.StopBits   ' ストップビット数
'----- ここまで
...
'書き込み
'Version1.80で全面変更
'タイムアウトを利用し，送信完了までフリーズ状態にならないように改良
Public Property Let Ascii(TxD As String)
    ...
    Dim StrTemp As String       '文字列変数
    Dim BitLen As Long          '全ビット長
    ...
    If GetCommState(ecH(Cn).Handle, fDCB) = False Then
        ...
        Select Case Xerror
            ...
            Case Is = 1 '-----トラップ可能エラー
                Err.Raise _
                    Number:=10103, _
                    Description:="Ascii - Write" & Chr$(&HA) & "ポート(" & Cn & ")の現在設定値(DCB)の読み込みに失敗しました"
                Exit Property
        End Select
    End If
    ...
    If ErrorFlag Then
        Select Case Xerror
            ...
            Case Is = 1 '-----トラップ可能エラー
                Err.Raise _
                    Number:=10104, _
                    Description:="Ascii - Write" & Chr$(&HA) & "ポート(" & Cn & ")のタイムアウトの読み出し，または設定に失敗しました．"
                Exit Property
        
        End Select
    End If
    
    StrTemp = StrConv(TxD, vbFromUnicode)   'ANSIに変換
    ' 送信ループ
    Do
        DoEvents
        bdata = StrTemp
...
' Version1.81で変更
' WriteFileの戻り値がWindowsのバージョンによって異なるため，エラーチェックをなくす．
...
'        End If
        ...
        If WriteBytes = WrittenBytes Then Exit Do   ' 全文字送信完了
...
'書き込み
'Version1.80で全面変更
'タイムアウトを利用し，送信完了まで待たされることが無いようにした
    ...
    If GetCommState(ecH(Cn).Handle, fDCB) = False Then
        ...
        Select Case Xerror
            ...
            Case Is = 1 '-----トラップ可能エラー
                Err.Raise _
                    Number:=10213, _
                    Description:="AsciiLine - Write" & Chr$(&HA) & "ポート(" & Cn & ")の現在設定値(DCB)の読み込みに失敗しました"
                Exit Property
        End Select
    End If
    ...
    If ErrorFlag Then
        Select Case Xerror
            ...
            Case Is = 1 '-----トラップ可能エラー
                Err.Raise _
                    Number:=10214, _
                    Description:="AsciiLine - Write" & Chr$(&HA) & "ポート(" & Cn & ")のタイムアウトの読み出し，または設定に失敗しました．"
                Exit Property
...
'----- ここまで
    ...
    StrTemp = StrConv(Td, vbFromUnicode)   'ANSIに変換
    ' 送信ループ
    Do
        DoEvents
        bdata = StrTemp
    ...
    If GetCommState(ecH(Cn).Handle, fDCB) = False Then
        ...
        Select Case Xerror
            ...
            Case Is = 1 '-----トラップ可能エラー
                Err.Raise _
                    Number:=10164, _
                    Description:="Binary - Write" & Chr$(&HA) & "ポート(" & Cn & ")の現在設定値(DCB)の読み込みに失敗しました"
                Exit Property
        End Select
    End If
    ...
    If ErrorFlag Then
        Select Case Xerror
            ...
            Case Is = 1 '-----トラップ可能エラー
                Err.Raise _
                    Number:=10165, _
                    Description:="Binary - Write" & Chr$(&HA) & "ポート(" & Cn & ")のタイムアウトの読み出し，または設定に失敗しました．"
                Exit Property
...
'Version1.84で変更．
'今後はEscapeCommFunctionを使用しない．
'EscapeCommFunctionは一時的な状態設定であって，DCBには反映されないことがわかった．
'RTS出力を恒久的に直接コントロールするには，DCBの書き換えを行う必要がある．
'    If Status = True Then
'        Stat = SETRTS
'      Else
'        Stat = CLRRTS
...
'今後はEscapeCommFunctionを使用しない．
'EscapeCommFunctionは一時的な状態設定であって，DCBには反映されないことがわかった．
'DCD出力を恒久的に直接コントロールするには，DCBの書き換えを行う必要がある．
'
'    If Status = True Then
'        Stat = SETDTR
'      Else
...
'SpecプロパティはApiのGetCommProperties関数をコールしていますが，踏査したところ，
'実際のハードウェアの状態によらず，かなりいい加減な値を返すことがわかっています．
'ソフト上からはどうしようもない問題なので，利用するときは必ず送受信波形で確認するなど
'が必要です．
'このあたりがレガシーインターフェースの泣き所かもしれません．
